...
impl CodeInterpreterTool {
    pub fn new() -> Self {
        Self {
            descriptor: ToolDescriptor::new("code_interpreter", "Execute code in a sandboxed environment (not yet implemented)").with_parameters(vec![
...
impl ToolExecutor for CodeInterpreterTool {
    ...
    async fn execute(&self, call: ToolCall) -> ToolResult {
        ToolResult::error(&call, "code_interpreter is not yet implemented (coming in Phase 5)")
    }
}
...
impl DelegateTool {
    pub fn new() -> Self {
        Self {
            descriptor: ToolDescriptor::new("delegate", "Delegate a task to a sub-agent (not yet implemented)").with_parameters(vec![
                ToolParameter::required("task", ParameterType::String, "Task description"),
                ToolParameter::optional("agent_profile", ParameterType::String, "Agent profile to use"),
            ]),
        }
    }
}
...
impl SkillsTool {
    pub fn new() -> Self {
        Self {
            descriptor: ToolDescriptor::new("skills", "List, read, and create skills (not yet implemented)").with_parameters(vec![
