...
fn gateway_catalog_contract_uses_file_backed_snapshot_or_defaults() {
    ...
    let matrix = records
        .iter()
        .find(|record| record.get("name").and_then(Value::as_str) == Some("matrix"))
        .expect("snapshot-backed gateway record should be present");
    assert_eq!(
        matrix.get("health").and_then(Value::as_str),
        Some("degraded"),
        "gateway catalog should prefer file-backed snapshot health"
    );
    assert_eq!(
        matrix.get("enabled_by_agent_count").and_then(Value::as_u64),
        Some(1),
