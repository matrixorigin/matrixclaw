...
impl OpenAiCompatibleProvider {
    ...
    async fn perform_completion(&self, request: &RunRequest) -> Result<ProviderResponse, ProviderError> {
        let response = self
            .client
...
fn request_messages(request: &RunRequest) -> Vec<Value> {
    let messages = if request.context_messages.is_empty() {
        vec![matrixclaw_agent_core::RunMessage::user(request.prompt.clone())]
    } else {
...
fn parse_provider_response(body: &Value) -> Result<ProviderResponse, ProviderError> {
    ...
    match content {
        Some(text) if !text.trim().is_empty() => Ok(ProviderResponse::text(text.trim().to_string())),
